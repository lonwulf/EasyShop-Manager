...
import com.lonwulf.labs.camera.domain.model.BarcodeConfirmingGraphic
import com.lonwulf.labs.camera.domain.model.InputInfo
import com.lonwulf.labs.camera.source.repository.FrameProcessorBase
import com.lonwulf.labs.camera.ui.camera.CameraReticleAnimator
...
class BarcodeProcessor(graphicOverlay: GraphicOverlay, private val cameraXViewModel: CameraXViewModel) :
    ...
    override fun onSuccess(
        ...
        if (!cameraXViewModel.isCameraLive.value) return
        ...
        if (barcodeInCenter == null) {
            ...
            if (sizeProgress < 1) {
                ...
                if (PreferenceUtils.shouldDelayLoadingBarcodeResult()) {
                    ...
                    cameraXViewModel.setWorkflowState(CameraXViewModel.WorkflowState.SEARCHING)
                } else {
                    cameraXViewModel.setWorkflowState(CameraXViewModel.WorkflowState.DETECTED)
                    cameraXViewModel.setDetectedBarcode(barcodeInCenter)
                }
            }
        }
        ...
        return ValueAnimator.ofFloat(0f, endProgress).apply {
            ...
            addUpdateListener {
                if ((animatedValue as Float).compareTo(endProgress) >= 0) {
                    graphicOverlay.clear()
                    cameraXViewModel.setWorkflowState(CameraXViewModel.WorkflowState.SEARCHED)
                    cameraXViewModel.setDetectedBarcode(barcode)
                } else {
                    graphicOverlay.invalidate()
                }
            }
