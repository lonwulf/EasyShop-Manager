...
package com.lonwulf.labs.camera.ui.objectDetection
...
import com.lonwulf.labs.camera.source.repository.ObjectConfirmationController
...
class MultiObjectProcessor(
    ...
    override fun onSuccess(
        ...
        var objects = results
        if (!cameraXViewModel.isCameraLive.value) {
            return
        }
        ...
        if (selectedObject != null) {
            cameraXViewModel.confirmingObject(selectedObject, confirmationController.progress)
        } else {
            cameraXViewModel.setWorkflowState(
                if (objects.isEmpty()) {
                    CameraXViewModel.WorkflowState.DETECTING
                } else {
                    CameraXViewModel.WorkflowState.DETECTED
                }
            )
        }
    }
