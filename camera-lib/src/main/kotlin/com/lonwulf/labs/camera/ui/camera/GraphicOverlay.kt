...
class GraphicOverlay(context: Context, attrs: AttributeSet?) : View(context, attrs) {
     ...
     * coordinates later.
     */
    fun setFrameMetadata(frameMetadata: FrameMetadata) {
        synchronized(lock) {
            if (frameMetadata.rotation == 90 || frameMetadata.rotation == 270) {
                previewWidth = frameMetadata.height
                previewHeight = frameMetadata.width
            } else {
                previewWidth = frameMetadata.width
                previewHeight = frameMetadata.height
            }
            this.isImageFlipped = frameMetadata.isImageFlipped
        }
    }
...
fun GraphicOverlayView(
    ...
    graphicOverlay: GraphicOverlay // The actual Android View instance
) {
    AndroidView(
